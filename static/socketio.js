...
$(document).ready(function(){
  ...
  socket.on('connect', function(){
    ...
    $(document).on('click', '#navigation a#new_file', function(){
      ...
      $('#navigation #tri_buttons').before('<a href="#" class="link"><code>新建文档...</code></a>');
    ...
    function cancelNewFile(){
      ...
      editingAllowed = true;
      $('#content #markdown_content').html('');
      $('#content #content_header h1').html('华邦云前端');
    ...
    $(document).on('click', '#navigation a#new_folder', function(){
      ...
      $('#content #content_header h1').html('新建文件夹');
      $('#content #markdown_content').html('按enter键执行新建');
      $('#navigation').children().attr('class', 'link');
      ...
      $('#navigation #tri_buttons').html('<a href="#" id="cancel_folder">' +
        '取消</a><a href="#" id="save_folder">新建文件夹</a>');
      changeContentHeight();
    });
  ...
  function showButtons(show, newFile){
    var buttons = '<a id="edit" href="#">编辑</a>\n<a id="save" href="#">保存</a>';
    if (show){
      if (newFile){
        $('#edit_save_buttons').html('<a id="cancel" href="#">取消</a>\n<a id="save" href="#">保存</a>');
      } else {
